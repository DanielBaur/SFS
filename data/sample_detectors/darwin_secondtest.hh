...
#ifndef darwin_secondtest_hh
#define darwin_secondtest_hh 1

#include "VDetector.hh"

using namespace std;

class darwin_secondtest : public VDetector {
 public:
  darwin_secondtest() {
    cerr << "*** Detector definition message ***" << endl;
    cerr << "You are currently using the darwin_secondtest detector."
         << endl
         << endl;
  ...
  virtual ~darwin_secondtest(){};

  // Do here the initialization of all the parameters that are not varying as a
  // function of time
  virtual void Initialization() {
    // Primary Scintillation (S1) parameters
    g1 = 9001.0;  // phd per S1 phot at dtCntr (not phe). Divide out 2-PE effect
    ...
    coinWind = 100.0;  // S1 coincidence window in ns
    coinLevel = 2.0;   // how many PMTs have to fire for an S1 to count
    numPMTs = 89.0;    // For coincidence calculation
    ...
    g1_gas = 0.0655;  // phd per S2 photon in gas, used to get SE size
    s2Fano = 3.61;   // Fano-like fudge factor for SE width
    s2_thr = 300.0;  // the S2 threshold in phe or PE, *not* phd. Affects NR most
    E_gas = 12.0;    // field in kV/cm between liquid/gas border and anode
    eLife_us = 2200.0;  // the drift electron mean lifetime in micro-seconds

    // Thermodynamic Properties
    inGas = false;
    T_Kelvin = 177.0;  // for liquid drift speed calculation
    ...
    dtCntr = 40.0;  // center of detector for S1 corrections, in usec.
    dt_min = 20.0;  // minimum. Top of detector fiducial volume
    dt_max = 60.0;  // maximum. Bottom of detector fiducial volume

    radius = 50.0;  // millimeters (fiducial rad)
    radmax = 50.0;  // actual physical geo. limit

    TopDrift = 150.0;  // mm not cm or us (but, this *is* where dt=0)
    ...
    cathode = 1.0;  // mm. Defines point below which events are gamma-X
